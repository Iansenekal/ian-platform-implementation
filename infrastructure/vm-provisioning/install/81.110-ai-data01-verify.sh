...
Runs 81.110 post-install verification checks for AI-DATA01.
Default env file:
  infrastructure/vm-provisioning/install/81.110-ai-data01-inputs.env
USAGE
}

ENV_FILE="infrastructure/vm-provisioning/install/81.110-ai-data01-inputs.env"
if [[ "${1:-}" == "--env-file" ]]; then
  ENV_FILE="${2:-}"
...
if [[ ! -f "${ENV_FILE}" ]]; then
  ...
  echo "Copy infrastructure/vm-provisioning/install/81.110-ai-data01-inputs.env.example first." >&2
  exit 1
fi
...
cat <<EOF2
81.110 AI-DATA01 verification completed successfully.
